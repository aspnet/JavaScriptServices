...
export function createWebpackDevServer(callback: CreateDevServerCallback, optionsJson: string) {
    ...
    if (path.extname(options.webpackConfigPath) === '.ts') {
        try {
            ...
            throw new Error('Error while attempting to enable support for Webpack config file written in TypeScript. Make sure your project depends on the "ts-node" NPM package. The underlying error was: ' + ex.messages + '\n' + ex.stack);
        }
    }
